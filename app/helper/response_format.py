...
class Json(BasePage[T], Generic[T]):
    metadata: MetadataSchema

    @classmethod
    def create(cls, code: str, message: str, data: Sequence[T], metadata: [Optional]):
        return cls(
            code=code,
            message=message,
            data=data
...
def Response(data) -> BasePage:
    code = '200'
    message = 'Success'

    try:
        return PageType.get().create(code, message, data)
